...
1. **Decide if a spec is warranted.** Yes if the task touches ≥2 modules / areas,
   ...
   is net-negative.
2. **Scaffold:** run
   `bash "${AGENT_HARNESS_ROOT:-.}/.agent/skills/new-spec/new-spec.sh" "<name>"` from the
...
4. **`context.md` `## 0. Resume here`** — write it with `handoff` (or `/agent:handoff`) at
   a stopping point; a fresh session resumes from it with `pickup` (or `/agent:pickup`).
