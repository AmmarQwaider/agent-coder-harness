...
description: >-
  ...
  Pairs with `/agent:pickup` or `pickup` (handoff writes, pickup reads).
argument-hint: "[note]"
...
3. Use the marked-block updater so a re-handoff replaces the block:
   `printf '%s' "$BLOCK" | bash "${AGENT_HARNESS_ROOT:-.}/.agent/skills/introspect/update-block.sh" <file> '<!-- resume:start' '<!-- resume:end -->'`
4. Print the exact `pickup` command (e.g. `/agent:pickup` or `.agent/skills/pickup`) to run in the fresh session.
